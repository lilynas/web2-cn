...
# 解析仓库名和用户名
IFS='/' read -r GITEE_USER GITEE_REPO <<< "$G_NAME"

# 构建 Gitee 仓库的克隆 URL，包含令牌
REPO_URL="https://${G_TOKEN}@gitee.com/${G_NAME}.git"
mkdir -p  ./data/gitee_data
# 克隆仓库
echo "正在克隆仓库……"
git clone "$REPO_URL" ./data/gitee_data || {
    echo "克隆失败，请检查 G_NAME 和 G_TOKEN 是否正确。"
    exit 1
}

if [ -f ./data/gitee_data/webui.db ]; then
    cp ./data/gitee_data/webui.db ./data/webui.db
    echo "从 Gitee 仓库中拉取成功"
else
    echo "Gitee 仓库中未找到 webui.db，将在同步时推送"
fi
